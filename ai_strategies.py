...
class AIAnalyzer:
    ...
    3. Grok/xAI (cloud, free tier) - console.x.ai
    """
    
    def __init__(self, 
                 backend: str = "ollama",  # "ollama", "groq", or "grok"
                 model: str = None,
                 api_key: str = None):
        ...
        if backend == "ollama":
            ...
            self.model = model or "grok-2-1212"  # or "grok-beta"
            self.api_key = api_key or os.getenv("XAI_API_KEY")
            self.base_url = "https://api.x.ai/v1"
        else:
            raise ValueError(f"Unknown backend: {backend}. Use 'ollama', 'groq', or 'grok'")
    ...
    def _query_openai_compatible(self, prompt: str, max_tokens: int = 150) -> Optional[str]:
        """Query OpenAI-compatible API (Groq or Grok/xAI)"""
        if not self.api_key:
            key_name = "GROQ_API_KEY" if self.backend == "groq" else "XAI_API_KEY"
            url = "console.groq.com" if self.backend == "groq" else "console.x.ai"
            print(f"⚠️ No {key_name} set. Get free key at {url}")
            self.enabled = False
            return None
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"{self.backend.upper()} error: {response.status_code} - {response.text[:100]}")
                return None
                
        except Exception as e:
            print(f"{self.backend.upper()} error: {e}")
            return None
    ...
    def query(self, prompt: str, max_tokens: int = 150) -> Optional[str]:
        ...
        else:  # groq or grok - both OpenAI compatible
            return self._query_openai_compatible(prompt, max_tokens)
...
class AIQuantStrategy(BaseStrategy):
    ...
    Usage:
        # With Ollama (local)
        strategy = AIQuantStrategy(backend="ollama")
        
        # With Grok (xAI cloud)
        strategy = AIQuantStrategy(backend="grok")
        
        # With Groq (cloud)
        strategy = AIQuantStrategy(backend="groq")
    """
    
    def __init__(self, use_ai: bool = True, backend: str = "ollama"):
        super().__init__("AIQuant")
        if use_ai:
            self.ai = AIAnalyzer(backend=backend)
        else:
            self.ai = None
...
def get_ai_strategy(name: str, backend: str = "ollama") -> BaseStrategy:
    """
    Get AI strategy with specified backend
    
    Args:
        name: Strategy name (aiquant, aitrend, aiquant_noai)
        backend: AI backend (ollama, groq, grok)
    """
    strategies = {
        "aiquant": AIQuantStrategy(use_ai=True, backend=backend),
        "aitrend": AITrendFollower(),
        "aiquant_noai": AIQuantStrategy(use_ai=False),
    }
...
def test_grok():
    """Test if Grok API key is set"""
    api_key = os.getenv("XAI_API_KEY")
    if api_key:
        print(f"✅ Grok API key found: {api_key[:10]}...")
        return True
    print("❌ No XAI_API_KEY set. Get free key at console.x.ai")
    return False
...
if __name__ == "__main__":
    ...
    print("=" * 50)
    print("Supports: Ollama (local), Groq (cloud), Grok/xAI (cloud)")
    print("=" * 50)
    ...
    print("🚀 SETUP OPTIONS:")
    print("=" * 50)
    print("\n1️⃣  LOCAL (Ollama + CodeLlama) - FREE, no internet needed")
    print("   ollama serve")
    print("   ollama pull codellama:7b")
    print("   python main.py --strategy aiquant")
    print("\n2️⃣  GROK (xAI) - FREE tier at console.x.ai")
    print("   export XAI_API_KEY=your_key_here")
    print("   python main.py --strategy aiquant")
    print("\n3️⃣  GROQ - FREE tier at console.groq.com")
