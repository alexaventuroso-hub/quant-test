...
class OmegaStrategy(BaseStrategy):
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.Series:
        ...
        bear_scores = df.apply(self._calculate_bear_score, axis=1)
        
        # Need score >= 12 out of 20 for entry (60% confirmation)
        min_score = 12
        
        # LONG: High bull score + in uptrend
        long_signal = (bull_scores >= min_score) & (df["ema_bull_score"] >= 3)
        
        # SHORT: High bear score + in downtrend
        short_signal = (bear_scores >= min_score) & (df["ema_bear_score"] >= 3)
        
        signals[long_signal] = 1
        signals[short_signal] = -1
        
        # Safety filter: Don't trade in choppy market
        signals[df["adx"] < 20] = 0
...
class TrendOnlyOmega(BaseStrategy):
    ...
    If EMAs say up, only takes longs
    If EMAs say down, only takes shorts
    No exceptions.
    """
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.Series:
        ...
        # STRICT trend requirement
        strong_uptrend = df["ema_bull_score"] >= 4  # 4/5 EMAs aligned bullish
        strong_downtrend = df["ema_bear_score"] >= 4
        
        # Only long in uptrend
        long_signal = (bull_scores >= 10) & strong_uptrend & (df["adx"] > 20)
        
        # Only short in downtrend
        short_signal = (bear_scores >= 10) & strong_downtrend & (df["adx"] > 20)
        
        signals[long_signal] = 1
        signals[short_signal] = -1
        
        # Never against trend - extra safety
        signals[long_signal & strong_downtrend] = 0
        signals[short_signal & strong_uptrend] = 0
    ...
    def get_signal(self, df: pd.DataFrame) -> TradeSignal:
        ...
        if current > 0:
            signal_type = Signal.BUY
            # Even wider stops for trend following
...
def get_omega_strategy(name: str) -> BaseStrategy:
    strategies = {
        "omega": OmegaStrategy(),
        "conservative": ConservativeOmega(),
        "trendonly": TrendOnlyOmega(),
    }
